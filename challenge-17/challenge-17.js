(function () {
  ...
  */
  ...
  console.log(
    text.replace(/Manuel Marques de Sousa/gi, "Hugo Cesar Firmino Costa")
  );
  ...
  */
  ...
  console.log(
    text.replace(/O Centauro de Luvas/gi, function (totalCapture) {
      return totalCapture.toLocaleUpperCase();
    })
  );
  ...
  */
  ...
  let months = ["Março", "Setembro", "dezembro"];

  for (let i = 0; i < months.length; i++) {
    console.log(
      `O mês de ${months[i]} é representado pelo número ${getMonthNumber(
        months[i]
      )}.`
    );
  }
  ...
  */
  ...
  let regexDate = /(\d{2}) de (\w+) de (\d{4})/gim;
  ...
  */
  ...
  function replaceDate(text) {
    return text.replace(regexDate, function (
      ...
      return `${capture1}/${getMonthNumber(capture2)}/${capture3}`;
    });
  }
