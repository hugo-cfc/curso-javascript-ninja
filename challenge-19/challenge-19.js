(function () {
  ...
  function hasClass(markup, cssClass) {
    let regexClass = new RegExp(`class=['"](${cssClass})['"]`, "gmi");
    return `${regexClass.test(markup)} para ${cssClass}.`;
  }
  ...
  let arrayClass = ["container", "text", "date", "excerpt", "main"];

  for (let i = 0; i < arrayClass.length; i++) {
    console.log(hasClass(markup, arrayClass[i]));
  }
